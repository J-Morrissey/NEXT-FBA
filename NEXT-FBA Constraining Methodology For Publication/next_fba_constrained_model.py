...
#NEXT-FBA Function
def next_fba_constrained_model(model,experiment,mapping_table,uptake_data,trained_c13bound_data,solver):
    # Extracting uptake reactions and NEXT-FBA constrained reactions
    uptake_reactions = list(uptake_data.index)
    ...
    for uptake_reaction in uptake_reactions:
        uptake_flux = uptake_data.loc[uptake_reaction, experiment + '_MEDIAN']
        if uptake_reaction in ['biomass_cho', 'biomass_cho_prod', 'ICproduct_Final_demand']:
            model.reactions.get_by_id(uptake_reaction).bounds = (uptake_flux, uptake_flux)
        else:
            model.reactions.get_by_id(uptake_reaction).bounds = (-uptake_flux, -uptake_flux)

    model.solver = solver

    # Methodology to prevent infeasibilities with median constraints
    model.reactions.get_by_id('biomass_cho_prod').bounds = (0, 0)
    model.reactions.get_by_id('biomass_cho').bounds = (0, 1000)
    model.reactions.get_by_id('ICproduct_Final_demand').bounds = (0, 1000)

    # Set objective to productivity
    model.objective = 'ICproduct_Final_demand'
    solution = model.optimize()
    optimal_prod = solution.objective_value

    # Constrain to optimal or experimental value based on productivity
    if optimal_prod <= uptake_data.loc['ICproduct_Final_demand', experiment + '_MEDIAN']:
        model.reactions.get_by_id('ICproduct_Final_demand').bounds = (optimal_prod, optimal_prod)
    else:
        model.reactions.get_by_id('ICproduct_Final_demand').bounds = (
            uptake_data.loc['ICproduct_Final_demand', experiment + '_MEDIAN'],
            uptake_data.loc['ICproduct_Final_demand', experiment + '_MEDIAN'])

    # Find max biomass at determined IGG flux
    model.objective = 'biomass_cho'
    solution = model.optimize()
    optimal_growth = solution.objective_value

    # Constrain to optimal or experimental value based on growth
    if optimal_growth <= uptake_data.loc['biomass_cho', experiment + '_MEDIAN']:
        model.reactions.get_by_id('biomass_cho').bounds = (optimal_growth, optimal_growth)
    else:
        model.reactions.get_by_id('biomass_cho').bounds = (
            uptake_data.loc['biomass_cho', experiment + '_MEDIAN'],
            uptake_data.loc['biomass_cho', experiment + '_MEDIAN'])
