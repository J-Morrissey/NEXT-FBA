...
def pca(X, ncomp, cross_val=False, cross_val_split=6, sample_groups=[], shsh=False, alpha=0.95):
    ...
    if isinstance(X, pd.DataFrame):
        x = X.to_numpy()
        sp_index = X.index.tolist()
    else:
        x = X.copy()
        sp_index = np.arange(len(X)).tolist()
    ...
    # Print cross-val results
    stats = pd.DataFrame({'component': np.arange(1, ncomp+1), 'eigenvalue': np.var(t, axis=0),
